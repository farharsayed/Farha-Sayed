...
def deviation(value,parent_pos,parent_neg):
    ...
    #we caculate the expected positives and expected negatives
    
    exp_pos = parent_pos*((value.pos+value.neg)/(parent_pos+parent_neg))
    exp_neg = parent_neg*((value.neg+value.pos)/(parent_neg+parent_pos))
    
    #and then use them to calculate deviation
    
    deviation = (((value.pos-exp_pos)**2)/(exp_pos)) + (((value.neg-exp_neg)**2)/(exp_neg))
    ...
    def decisiontree_iterator(parent):
        ...
        if further_fork == False:
            ...
            if(parent.pos > 0 and parent.neg > 0):
                ...
                #we calculate the p value for the value of delta
...
def learn_tennis_tree(filename):
    ...
    #creaing a data set using the DataSet constructor
...
def test_tennis_tree(filename):
    ...
    tennis_dataSet = DataSet( name = filename , target = 'Play' ) 
    
    #splitting data into training (80%) and test(20%) sets
    trainSet, testSet = train_test_split(tennis_dataSet , test_split = 0.2)
    
    
    tree = DecisionTreeLearner(trainSet)
    
    #evaluating performance of the tree by calculating the error ratio
    error = err_ratio(tree , testSet)
    
    #function returns the training set, the test set, the learned decision tree and the error rate achieved
...
def genSyntheticTrainSet():
    ...
    #generating 200 data examples from this tree
...
def genSyntheticTestSet():
    ...
    #creating a dataset of 100 examples
...
def train_restaurant_tree(trainSet, testSet, N=200):
    ...
    error1 = False
    tree1 = DecisionTreeLearner(trainSet)
    
    errorcalculated = err_ratio(tree1, testSet)
    
    for a in range (0,N):
        splitset = 1- ((a+1)/200)
        data, extra = train_test_split(trainSet,test_split=(splitset))
        tree = DecisionTreeLearner(data)
        
        error = err_ratio(tree,testSet)
        if(error == errorcalculated and error1==False):
            error1 = True
            samples_required = a
...
def train_tree(trainSet, testSet):
    ...
    #learning a decision tree using the whole training set created in 3a
    tree = DecisionTreeLearner(trainSet)
    
    #evaluating performance of the tree using the error ratio
    error = err_ratio(tree,testSet)
